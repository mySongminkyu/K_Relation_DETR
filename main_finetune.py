...
def train():
    ...
    # ────────────────────────────────────────────────────────────────
    #   DEBUG:  pretrained_weight 로드 검증 --------------------------
    #   1)  모델 안에서 Conv2d 하나를 찾고  weight 백업
    #   2)  ckpt 로드
    #   3)  같은 weight 와 비교 →  바뀌면 ✔ True
    # ----------------------------------------------------------------
    ...
    conv0 = _first_conv(model)
    w_before = conv0.weight.clone()

    if hasattr(cfg, "pretrained_weight") and os.path.isfile(cfg.pretrained_weight):
        ckpt = torch.load(cfg.pretrained_weight, map_location="cpu")
        load_state_dict(model, ckpt)              # strict=False 내부 처리
        logger.info(f"✅  loaded pretrained_weight: {cfg.pretrained_weight}")
    elif (wp := getattr(cfg, "resume_from_checkpoint", None)) and os.path.isfile(wp):
        load_state_dict(model, load_checkpoint(wp))
        logger.info(f"✅  loaded resume_from_checkpoint: {wp}")
    else:
        logger.warning("⚠️  NO pretrained weight / resume checkpoint given!")

    w_after = conv0.weight
    logger.info(f"🔍  weight changed after load: {not torch.equal(w_before, w_after)}")
    # ────────────────────────────────────────────────────────────────
